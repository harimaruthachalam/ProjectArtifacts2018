            ...
            for iterStandandize = 1 : length(standandize)
                for iterTopC = 1 : length(topC)
                    for iterTransorm = 1 : length(transorm)
                        ...
                        Main(0,1,interpolate(iterInterpolate),0,50,40,1,feature(iterFeature),standandize(iterStandandize),topC(iterTopC), transorm(iterTransorm),trainPercent(iterTrainPercent));
                    end
